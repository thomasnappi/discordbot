...
class Annoying(commands.Cog):
    ...
    async def remind(self, ctx, user: discord.Member, hours: float, message: str):
        ...
        await asyncio.sleep(round(360 * hours))
        await ctx.send("{0.mention}, {1.author.name}#{1.author.discriminator} asked me {2} hours ago to tell you {3}".format(user,ctx.author,hours,message))
